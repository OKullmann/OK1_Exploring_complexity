...
\item Complete Figure \ref{fig:basicmeasurement}.
  ...
  \end{enumerate}
\item Complete Lemma \ref{lem:basicmeasurements}.
\item Complete Lemma \ref{lem:basicmeasurements2}.
\end{enumerate}
...
\begin{defi}\label{def:php}
  ...
  \begin{eqnarray*}
    ...
    F_{\le 1} & := & \setb{ \set{\ol{p_{i_1,j}},\ol{p_{i_2,j}}} \mb i_1,i_2 \in \tb1m, i_1 < i_2, j \in \tb1k }.
  \end{eqnarray*}
...
\begin{lem}\label{lem:basicmeasurements}
  Consider $m, k \in \NNZ$.
  \begin{enumerate}
  \item $c(F^{\ge 1}) = m$ (the ``long clauses'').
  \item $c(F_{\le 1}) = \frac {n(n+1)n}{2}$ (the `` short clauses'').
  \item
  \item 
  \end{enumerate}
\end{lem}
\begin{prf}
XXX since we have a double-nested loop selecting two different cells in the same column with selection order being irreverent. \Qed
\end{prf}

\begin{lem}\label{lem:basicmeasurements2}
  Consider $m, k \in \NNZ$.
  \begin{enumerate}
  \item $n(\php^m_k) = n(\fphp^m_k) XXX = XXX$
  \item $c(\php^m_k) = XXX$
  \item
  \item
  \item 
